...
parser.add_argument(
    "--graph", required=True, type=str, help="Absolute path to graph file (.pb)"
)
parser.add_argument(
    "--labels", required=True, type=str, help="Absolute path to labels file (.txt)"
)
parser.add_argument(
    "--output_layer",
    type=str,
...
parser.add_argument(
    "--roi_size", type=int, default=32, help="Size, in pixels, of the occluding filter"
)
parser.add_argument(
    "--stride",
    type=int,
...
def createOcclusion(im, roi_size, stride_size):
    ...
    for r in range(0, iters):
        for c in range(0, iters):
            imOc = np.copy(im)
            imOc[
                r * stride_size : r * stride_size + roi_size - 1,
                c * stride_size : c * stride_size + roi_size - 1,
            ] = 150
...
def run_graph(
    ...
    with tf.Session() as sess:
        ...
        (predictions,) = sess.run(
            softmax_tensor, {input_layer_name: occlusion_array_string.getvalue()}
        )
        ...
        map_filename = "/home/daniel/Documents/maps/{}.png".format(
        ...
        print(
            "Finished in {} minutes and {} seconds".format(elapsed // 60, elapsed % 60)
        )
        plt.close("all")
        return 0
